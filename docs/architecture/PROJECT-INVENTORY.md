...
| 13 | `vercel.json` | OK [VERIFIED] | `vercel.json` (22 líneas) | LOW | `framework: nextjs`; `buildCommand: pnpm build`; **2 crons diarios** `/api/cron/siem` (`0 3 * * *`) y `/api/cron/uptime` (`30 3 * * *`), ambos con `CRON_SECRET` (Hobby: máx. 1 ejecución/día); `regions: [iad1]`. |
...
| INCONSISTENCIA-4 | 20 migraciones (plan B00) | 22 archivos `.sql` en `drizzle/` (journal 22, última `0021_push_active_boolean`) | **RESUELTA (2026-08-08):** 22/22 disco=journal; el huérfano `0001_quota_enforcement.sql` fue eliminado en higiene B03/MODE C. |
