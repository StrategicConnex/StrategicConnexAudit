...
| 13 | `vercel.json` | OK [VERIFIED] | `vercel.json` (22 líneas) | LOW | `framework: nextjs`; `buildCommand: pnpm build`; **2 crons** `/api/cron/siem` (`*/5 * * * *`) y `/api/cron/uptime` (`*/15 * * * *`), ambos con `CRON_SECRET`; `regions: [iad1]`. |
