...
export type SeoReportOutcome =
  | { ok: true; report: string; isFallback: boolean; modelUsed?: string; fromCache?: boolean }
  | { ok: false; error: string; status: number };

/**
 * seo-report-service.ts — Generación de informe SEO compartida (P2-1).
 *
 * La usan la ruta síncrona (fallback local cuando Trigger.dev no está) y
 * el task `ai-seo-report-generation` (camino diferido por defecto).
 */
export async function generateSeoReport(
  projectId: string,
  userId: string
): Promise<SeoReportOutcome> {
  try {
    ...
    return { ok: true, report: fallbackReport, isFallback: true };
  } catch (error) {
